...
- ✅[2026-01-27] 初始化git仓库
- ✅创建Python虚拟环境，安装fastapi, sqlalchemy等基础包

### 任务1.2：编写配置文件（config.py）和环境变量示例

- 创建配置类，从环境变量读取配置（如数据库URL，AI API密钥等）
- 创建.env.example文件，列出需要的环境变量

### 任务1.3：设计数据库模型（models.py）和数据库连接（database.py）

- 使用SQLAlchemy定义stock、新闻、信号等模型
- 创建数据库连接引擎和会话本地类

### 任务1.4：创建Pydantic模型（schemas.py）用于请求和响应

- 定义API接口中使用的数据模型，如stock数据返回模型、信号模型等

### 任务1.5：编写FastAPI主应用（main.py）并测试运行

- 创建FastAPI应用实例
- 配置中间件、路由等
- 编写一个简单的测试接口，如/health
